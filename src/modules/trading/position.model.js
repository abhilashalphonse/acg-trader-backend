...
positionSchema.pre('validate', function validatePositionState(next) {
  try {
    ...
    if (this.trailing?.enabled && (this.trailing.distancePoints == null || compareDecimal(this.trailing.distancePoints, '0') <= 0)) {
      this.invalidate('trailing.distancePoints', 'Enabled trailing stop requires a positive distancePoints');
    }
    if (this.trailing?.bestPrice != null && compareDecimal(this.trailing.bestPrice, '0') <= 0) this.invalidate('trailing.bestPrice', 'bestPrice must be greater than zero');
  } catch (error) {
  ...
  if (this.status === 'CLOSED') {
    ...
    if (this.openVolume != null && compareDecimal(this.openVolume, '0') !== 0) this.invalidate('openVolume', 'Closed positions must have zero openVolume');
  }
  if (this.status === 'OPEN') {
