...
const trailingSchema = new Schema({ enabled: { type: Boolean, default: false }, distancePoints: { type: Decimal128, default: null }, bestPrice: { type: Decimal128, default: null }, activatedAt: { type: Date, default: null } }, { _id: false });
...
applyTenantScope(positionSchema);
positionSchema.index({ tenantId: 1, accountId: 1, status: 1, openedAt: -1 });
positionSchema.index({ tenantId: 1, accountId: 1, symbol: 1, status: 1 });
...
positionSchema.pre('validate', function validatePositionState(next) {
  try {
    ...
    if (this.initialVolume != null && this.openVolume != null && compareDecimal(this.openVolume, this.initialVolume) > 0) this.invalidate('openVolume', 'openVolume cannot exceed initialVolume');
    ...
    if (this.status === 'CLOSED') { if (!this.closedAt) this.invalidate('closedAt', 'Closed positions require closedAt'); if (this.openVolume != null && compareDecimal(this.openVolume, '0') !== 0) this.invalidate('openVolume', 'Closed positions require zero openVolume'); if (this.margin != null && compareDecimal(this.margin, '0') !== 0) this.invalidate('margin', 'Closed positions require zero margin'); }
    if (this.status === 'OPEN') { if (this.closedAt) this.invalidate('closedAt', 'Open positions cannot have closedAt'); if (this.openVolume != null && compareDecimal(this.openVolume, '0') <= 0) this.invalidate('openVolume', 'Open positions require positive openVolume'); }
    if (this.trailing?.enabled && (this.trailing.distancePoints == null || compareDecimal(this.trailing.distancePoints, '0') <= 0)) this.invalidate('trailing.distancePoints', 'Enabled trailing stop requires positive distancePoints');
  } catch (error) { this.invalidate('openVolume', error.message); }
  next();
});
