...
function createTradingRuntime({ marketRuntime }) {
  ...
  const riskDayEngine = new RiskDayEngine({ eventBus, logger });
  ...
  return {
    ...
    eventBus,
    marketOrderService, atomicReverseService, tradingCommandService, accountControlService, accountLedgerService, platformEventRelay, pendingOrderService, pendingOrderAmendService, tradingHistoryService, positionProtectionService, trailingStopService, valuationEngine, currencyConversionEngine, protectionTriggerEngine, pendingOrderEngine, trailingStopEngine, riskDayEngine, reconciliationService, commandQueue,
    async start() {
      if (started) return;
