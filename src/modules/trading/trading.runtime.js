...
function createTradingRuntime({ marketRuntime }) {
  ...
  const valuationEngine = new ValuationEngine({ quoteStore: marketRuntime.quoteStore, eventBus: marketRuntime.eventBus, currencyConverter: currencyConversionEngine, logger });
  ...
  const atomicReverseService = new AtomicReverseService({ quoteStore: marketRuntime.quoteStore, eventBus: marketRuntime.eventBus, commandQueue, idempotencyService, valuationEngine, logger });
  ...
  const pendingOrderAmendService = new PendingOrderAmendService({ quoteStore: marketRuntime.quoteStore, eventBus: marketRuntime.eventBus, commandQueue, idempotencyService, logger });
  ...
  return {
    ...
    health() {
      ...
      return {
        ...
        valuation: valuationEngine.health(), platformEvents: platformEventRelay.health(), protection: protectionTriggerEngine.health(), pendingOrders: pendingOrderEngine.health(), trailing: trailingStopEngine.health(), reconciliation,
        capabilities: {
          ...
          platformEventRelay: true, signedPlatformWebhooks: true, instrumentMaster: true, tradingSessions: true, tradingHolidays: true,
          accountCurrencyConversion: true, crossCurrencyMargin: true, crossCurrencyPnl: true,
          marketOpen: true, marketClose: true, partialClose: true, serverReverse: true, atomicReverse: true, serverCloseAll: true, realtimeValuation: true, accountEquity: true,
          pendingOrders: true, limitOrders: true, stopOrders: true, stopLimitOrders: true, pendingOrderExpiry: true, pendingOrderCancel: true, pendingOrderAmend: true,
          durableHistory: true, paginatedOrders: true, paginatedDeals: true, paginatedPositions: true, challengeRiskPolicy: true,
    ...
    async stop() {
      reconciliationService.stopPeriodic(); await trailingStopEngine.stop(); await pendingOrderEngine.stop(); await protectionTriggerEngine.stop(); await platformEventRelay.stop(); await commandQueue.drainAll(); await valuationEngine.stop(); setDefaultCurrencyConversionEngine(null); started = false;
    },
  };
}
