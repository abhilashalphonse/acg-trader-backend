...
function createTradingRuntime({ marketRuntime }) {
  ...
  const challengeRiskEngine = new ChallengeRiskEngine({ eventBus, valuationEngine, accountControlService, commandQueue, logger });
  ...
  return {
    ...
    eventBus,
    marketOrderService, atomicReverseService, tradingCommandService, accountControlService, challengeRiskEngine, accountLedgerService, platformEventRelay, pendingOrderService, pendingOrderAmendService, tradingHistoryService, positionProtectionService, trailingStopService, valuationEngine, currencyConversionEngine, protectionTriggerEngine, pendingOrderEngine, trailingStopEngine, reconciliationService, commandQueue,
    async start() {
      if (started) return;
      ...
      try {
        ...
        try {
          ...
          } catch (error) { await challengeRiskEngine.stop(); throw error; }
