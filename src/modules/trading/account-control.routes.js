...
const riskPolicy = z.object({ dailyLoss: limitRule.optional(), maxLoss: limitRule.optional(), profitTarget: decimalInput.optional(), breachAction: z.enum(['LOCK_ONLY', 'CANCEL_ORDERS_AND_LOCK', 'LIQUIDATE_AND_LOCK']).optional(), maxOpenPositions: z.number().int().positive().nullable().optional(), maxPendingOrders: z.number().int().positive().nullable().optional(), maxPositionVolume: nullableDecimal, maxSymbolVolume: nullableDecimal, maxTotalVolume: nullableDecimal, requireStopLoss: z.boolean().optional(), maxRiskPerTradePercent: nullableDecimal, maxAggregateRiskPercent: nullableDecimal, allowedSymbols: z.array(z.string().trim().min(1).max(32)).optional() }).strict().optional();
const riskPolicyPatch = z.object({ dailyLoss: limitRule.optional(), maxLoss: limitRule.optional(), profitTarget: decimalInput.optional(), breachAction: z.enum(['LOCK_ONLY', 'CANCEL_ORDERS_AND_LOCK', 'LIQUIDATE_AND_LOCK']).optional(), maxOpenPositions: z.number().int().positive().nullable().optional(), maxPendingOrders: z.number().int().positive().nullable().optional(), maxPositionVolume: patchNullableDecimal, maxSymbolVolume: patchNullableDecimal, maxTotalVolume: patchNullableDecimal, requireStopLoss: z.boolean().optional(), maxRiskPerTradePercent: patchNullableDecimal, maxAggregateRiskPercent: patchNullableDecimal, allowedSymbols: z.array(z.string().trim().min(1).max(32)).optional() }).strict().optional();
...
function createAccountControlRouter(runtime, authService) {
  ...
  router.patch('/:accountId/challenge', async (req, res) => {
    ...
    const account = await runtime.commandQueue.run(accountId, async () => {
      ...
      for (const key of ['profitTarget','breachAction','maxOpenPositions','maxPendingOrders','maxPositionVolume','maxSymbolVolume','maxTotalVolume','requireStopLoss','maxRiskPerTradePercent','maxAggregateRiskPercent','allowedSymbols']) if (patch.riskPolicy && patch.riskPolicy[key] !== undefined) doc.riskPolicy[key] = patch.riskPolicy[key];
