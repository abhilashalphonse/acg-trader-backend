...
function createAccountControlRouter(runtime, authService) {
  ...
  router.patch('/:accountId/challenge', async (req, res) => {
    ...
    const account = await runtime.commandQueue.run(accountId, async () => {
      ...
      for (const key of ['profitTarget','breachAction','maxOpenPositions','maxTotalVolume','allowedSymbols']) if (patch.riskPolicy && patch.riskPolicy[key] !== undefined) doc.riskPolicy[key] = patch.riskPolicy[key];
