...
class CurrencyConversionEngine {
  ...
  #convertLeg(amount, from, to, nowMs) {
    ...
    if (this.#symbolAvailable(directSymbol)) {
      ...
      if (quote) {
        const rate = normalizeDecimal(String(positive ? quote.bid : quote.ask));
    ...
    if (this.#symbolAvailable(inverseSymbol)) {
      ...
      if (quote) {
        const divisor = normalizeDecimal(String(positive ? quote.ask : quote.bid));
        return divideDecimal(amount, divisor, { scale: 12 });
      }
    }
    return null;
  }

  #symbolAvailable(symbol) {
    return !this.symbols.size || this.symbols.has(symbol);
  }
  ...
  #quote(symbol, nowMs) {
    ...
    if (!Number.isFinite(receivedAtMs) || nowMs - receivedAtMs > this.maxQuoteAgeMs) return null;
    return quote;
  }
}
...
function conversionUnavailable(from, to) {
  return new AppError('Live currency conversion path is unavailable', {
    statusCode: 409,
...
module.exports = { CurrencyConversionEngine, normalizeCurrency };
