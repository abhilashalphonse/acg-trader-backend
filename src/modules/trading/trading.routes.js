...
const openSchema = z.object({ accountId: objectId, clientOrderId: z.string().trim().min(1).max(128), symbol: z.string().trim().min(1).max(32), side: z.enum(['BUY', 'SELL']), volume: decimalInput, stopLoss: optionalDecimal, takeProfit: optionalDecimal, requestedPrice: optionalDecimal, source: z.enum(['WEB', 'MOBILE', 'API']).optional().default('API') }).strict();
const pendingSchema = z.object({ accountId: objectId, clientOrderId: z.string().trim().min(1).max(128), symbol: z.string().trim().min(1).max(32), side: z.enum(['BUY', 'SELL']), type: z.enum(['LIMIT', 'STOP', 'STOP_LIMIT']), volume: decimalInput, limitPrice: optionalDecimal, stopPrice: optionalDecimal, stopLoss: optionalDecimal, takeProfit: optionalDecimal, timeInForce: z.enum(['GTC', 'TODAY', 'SPECIFIED']).optional().default('GTC'), expiresAt: z.union([z.string().datetime({ offset: true }), z.null()]).optional().default(null), source: z.enum(['WEB', 'MOBILE', 'API']).optional().default('API') }).strict();
const closeSchema = z.object({ accountId: objectId, clientOrderId: z.string().trim().min(1).max(128), volume: optionalDecimal, requestedPrice: optionalDecimal, source: z.enum(['WEB', 'MOBILE', 'API']).optional().default('API') }).strict();
...
function createTradingRouter(runtime, authService) {
  ...
  router.get('/accounts/:accountId/valuation', async (req, res) => {
  ...
  router.get('/positions/:positionId/valuation', async (req, res) => {
  ...
  router.get('/accounts/:accountId/orders/pending', async (req, res) => {
    const accountId = parseObjectId(req.params.accountId);
    requireAccountGrant(req.traderPrincipal, accountId);
    res.json({ orders: await runtime.pendingOrderService.listPendingOrders(accountId) });
  });
  ...
  router.post('/orders/:orderId/cancel', async (req, res) => {
    ...
    const command = parse(cancelPendingSchema, req.body);
    const accountId = await orderAccountId(orderId);
    assertResourceAccount(accountId, command.accountId, 'ORDER_ACCOUNT_MISMATCH');
    requireAccountGrant(req.traderPrincipal, accountId);
    const result = await runtime.pendingOrderService.cancelPendingOrder({ ...command, orderId });
    res.status(result.idempotentReplay ? 200 : 201).json(result);
  });
  ...
  function positionCommand(executor, schema, createdResponse = false) {
    return async (req, res) => {
      ...
      assertResourceAccount(accountId, command.accountId, 'POSITION_ACCOUNT_MISMATCH');
      requireAccountGrant(req.traderPrincipal, accountId);
      const result = await executor({ ...command, positionId });
...
async function orderAccountId(orderId) {
  const order = await Order.findById(orderId).select('accountId').lean();
  if (!order) throw new AppError('Order was not found', { statusCode: 404, code: 'ORDER_NOT_FOUND' });
  return String(order.accountId);
}
async function positionAccountId(positionId) {
  const position = await Position.findById(positionId).select('accountId').lean();
  if (!position) throw new AppError('Position was not found', { statusCode: 404, code: 'POSITION_NOT_FOUND' });
  return String(position.accountId);
}
