...
class PlatformEventRelay {
  ...
  async enqueueDeal({ account, deal, session = null } = {}) {
    ...
    return this.#enqueue(account, fundedAccountId, 'DEAL_CREATED', {
      ...
      referencePrice: decimal(deal.referencePrice),
      executionBid: decimal(deal.executionBid),
      executionAsk: decimal(deal.executionAsk),
      spreadPoints: decimal(deal.spreadPoints),
      providerSpreadPoints: decimal(deal.providerSpreadPoints),
      liquidityAdjustmentPoints: decimal(deal.liquidityAdjustmentPoints),
      volumeBand: deal.volumeBand || null,
  ...
  async #captureDeal(payload) {
    ...
    await this.#enqueue(account, fundedAccountId, 'DEAL_CREATED', {
      ...
      referencePrice: decimal(payload?.referencePrice),
      executionBid: decimal(payload?.executionBid),
      executionAsk: decimal(payload?.executionAsk),
      spreadPoints: decimal(payload?.spreadPoints),
      providerSpreadPoints: decimal(payload?.providerSpreadPoints),
      liquidityAdjustmentPoints: decimal(payload?.liquidityAdjustmentPoints),
